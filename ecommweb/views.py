...
from flask import Blueprint,render_template, flash,redirect,request, jsonify, redirect
...
def add_to_cart(item_id):
    ...
    if item_exists:
        try:
            ...
            flash(f'{item_exists.product.product_name} not updated')
            return redirect(request.referrer)
    ...
    try:
        ...
        flash(f'{new_cart_item.product.product_name} not been added to cart, Plz Try Again')
    return redirect(request.referrer)   
    ...
    if customer_cart:
        try:
            ...
            for item in customer_cart:
                ...
                new_order.price = item.product.current_price  # Fixed: use price instead of current_price
                ...
                product.in_stock = product.in_stock - item.quantity  # Fixed: subtract quantity
            ...
            flash('Order could not be placed , Try again')
            return redirect('/')
    else:
